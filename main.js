...
function arslanLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [𝐌𝐎𝐄𝐄𝐃𝐱𝐓𝐄𝐂𝐇] ${new Date().toISOString()}: ${message}`);
}
    ...
    try {
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const newsletterJids = ['120363426281901729@newsletter'];
                const newsEmojis = ['❤️', '👍', '😮', '😎', '💀', '💫', '🔥', '👑'];
                ...
                const myquoted = {
                    key: { remoteJid: 'status@broadcast', participant: '120363426281901729@newsletter', fromMe: false, id: createSerial(16).toUpperCase() },
                    message: { contactMessage: {
                        displayName: '© MOEED-MD',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:MOEED-MD BOY\nORG:MOEED-MD BOY;\nTEL;type=CELL;type=VOICE;waid=13135550002:13135550002\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© MOEED-MD' } }
                    }},
        ...
        arslanLog(`MOEED-MD Pair error: ${err.message}`, 'error');
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: 'Moeed-md is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 MOEED-MD — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
